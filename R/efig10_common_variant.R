...
enrichment_corrections = function(save_plot=F) {
  ...
  enrichment_correction_data = sum_stats %>%
  ...
  if (save_plot) {
    pdf('e9b_enrichment_corrections.pdf', height=3, width=4)
    print(p)
    dev.off()
  }
  return(p)
...
window_assessment = function(save_plot=F) {
  window_data = sum_stats %>%
    filter(cond %in% c('all100', 'all50', 'all10') &
             grepl('oe_lof_upper_quantile_', name) &
             (is.na(cor_rm0.2) | cor_rm0.2 == 0)  # Select uncorrelated variables
    ) %>%
    group_by(name, cond) %>%
    summarise(meta_enrichment = metagen(enrichment, se)$TE.random,
              meta_sd = metagen(enrichment, se)$seTE.random) %>%
    mutate(decile = as.integer(str_sub(name, -1)),
